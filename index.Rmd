...
date: "Workshop on Missing Data & Survival Analysis<br>June 1st, 2022"
...
# script = ErasmusMC::get_speaker_notes("MDSA2022.html", export = FALSE)
...
In addition, we had 8 additional, time-constant covariates, of which 4 had
...
The whole concept of multiple imputation was developed by Donald Rubin in the
1970s and he realized that imputed values should be sampled from the predictive
...
That means we need to sample the imputed values for an incomplete covariate $x$
from a distribution for $x$ conditional on everything else, where "everything
else" includes the outcome/response, other covariates, and parameters.
...
Let's have a look at this in a very simple setting where we have a response $y$,
an incomplete covariate $x_1$ and two completely observed covariates $x_2$ and
$x_3$.
...
but in **multiple variables**, and the **predictive distribution** of the
missing values conditional on everything else **is then multivariate**.
...
Because in these imputation models condition on unobserved values, we use an
iterative algorithm that starts by randomly drawing initial values from the
observed part of the data, and then cycles through the incomplete variables and
impute one at a time.

This algorithm is based on the idea of the Gibbs sampler.
...
When we use the Cox model, we model the hazard function, and here I consider
only the simple setting in which we do not have time-varying covariates.

$\mathbf X$ refers to the design matrix which contains completely observed and
incomplete covariates.


But the data that we use to fit such a model, does not contain the hazard as
a response variable. Instead, we have the time of the event or censoring, 
...
When we now use the MICE algorithm on such survival data, usually the following happens:
...
The model for $x_1$, for example, would contain the event Time and event indicator
and all other covariates in its linear predictor.

At first glance, this seems OK, because we have included "everything else", 
meaning the outcome and the other covariates.

But this model assumes that there is a linear association between $x_1$ and the
event time and event indicator.
...
Our analysis model implies that there is a non-linear association between
covariates and the event time and indicator.

The implication of this becomes more clear when we look at a simpler example
that can be visualized more easily.
...
p
# knitr::include_graphics("figures/linplot.png")
```
...
We just simplify the setting back to the situation where we have a standard
univariate response variable $y$ and we want to impute the incomplete covariate
$x_1$ and we assume that both are continuous and conditionally normally distributed.
Then we could the linear regression model for $x_1$ with $y$ and other covariates
as imputation model and this model of course assumes that there is a linear
...
When analysing the data, so when we are thinking about the model for $y$ in which
$x_1$ is a covariate, we might assume that there is a non-linear association, 
for example, that $x_1$ has a quadratic effect on $y$.
...
Recurrence of PSC and graft number are **time-varing variables**:
<ul class = "arrowlist">
...
of 
* the distribution of $x_4$ conditional on $x_1$, $x_2$ and $x_3$,
* the distribution of $x_3$ conditional on $x_1$ and $x_2$, and
...
We can easily add multiple levels of hierarchy by specifying additional 
random effects terms.

Because the outcome is on the graft level, but covariates are on the patient
level, this will now actually add a random intercept to the linear predictor
